...
$path='../themes/upload/students/';$w=200;$h=200; ?>
...
<tr>
  <td>Type</td>
  <td><select name="msg_menu_id"<?php if(isset($q_ed)){?> title="<?=$q_ed->msg_menu_id?>"<?php }?> >
          <?php $fuqw=mysql_query("select * from  sy_menu where menu_cat_type=8 order by menu_nm"); while($rty=mysql_fetch_object($fuqw)){?>
          <option value="<?=$rty->menu_id?>"><?=$rty->menu_nm?></option><?php } ?></select></td>
</tr>
<tr>
  <td>Name</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="8">
  ...
  <?php $qw= "select * from $tbl,sy_menu where menu_id=msg_menu_id order by $expo_key[0] desc ".$per_limit;
